...
interface PermitHazardCategoryItem {
  id: number;
  type: string;
  activity: string;
  subActivity: string;
...
const PermitHazardCategory: React.FC = () => {
  ...
  const [data, setData] = useState<PermitHazardCategoryItem[]>([
    {
      id: 1,
      type: "Hot Work",
  ...
  const handleSubmit = () => {
    if (!newType || !newActivity || !newSubActivity || !newHazardCategory) {
  ...
  const handleEdit = (item: PermitHazardCategoryItem) => {
    setIsAdding(true);
  ...
  const filteredData = data.filter((item) => {
    ...
    return matchesGlobalSearch && matchesTypeFilter && matchesActivityFilter && 
           matchesSubActivityFilter && matchesHazardCategoryFilter;
  });
    ...
    <div className="p-4 bg-white">
      <div className="flex justify-between items-center">
        <TopSearch
          ...
          buttons={["New Permit Hazard Category"]}
        />
        <Pagination
