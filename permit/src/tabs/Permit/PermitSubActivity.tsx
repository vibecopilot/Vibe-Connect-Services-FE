...
interface PermitSubActivityItem {
  id: number;
  type: string;
  activity: string;
  subActivity: string;
...
const PermitSubActivity: React.FC = () => {
  const [search, setSearch] = useState("");
  const [typeSearchTerm, setTypeSearchTerm] = useState("");
  const [activitySearchTerm, setActivitySearchTerm] = useState("");
  const [subActivitySearchTerm, setSubActivitySearchTerm] = useState("");
  const [showFilterRow, setShowFilterRow] = useState(false);
  const [data, setData] = useState<PermitSubActivityItem[]>([
    { id: 1, type: "Hot Work", activity: "CCTV install", subActivity: "Cable laying" },
    { id: 2, type: "Cold Work", activity: "Water supply", subActivity: "Pipe fitting" },
  ]);
  ...
  const handleSubmit = () => {
    if (!newType || !newActivity || !newSubActivity) {
    ...
    if (editId !== null) {
      setData((prev) =>
        prev.map((item) =>
          item.id === editId
            ? { ...item, type: newType, activity: newActivity, subActivity: newSubActivity }
            : item
        )
      );
      setEditId(null);
  ...
  const handleEdit = (item: PermitSubActivityItem) => {
    setIsAdding(true);
  ...
  const filteredData = data.filter((item) => {
    ...
    return matchesGlobalSearch && matchesTypeFilter && matchesActivityFilter && matchesSubActivityFilter;
  });
    ...
    <div className="p-4 bg-white ">
      <div className="flex justify-between items-center">
        <TopSearch
          ...
          buttons={["New Permit Sub Activity"]}
        />
        <Pagination
              ...
              <tr >
                ...
                <td className="p-2 border-b border-gray-400">
                  <input
                    ...
                    placeholder="Search Permit Sub Activity"
                    className="w-full px-2 py-1 text-sm border rounded border-gray-300"
                  />
                </td>
              </tr>
...
export default PermitSubActivity; 