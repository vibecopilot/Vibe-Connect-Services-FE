...
const ManageFloor: React.FC = () => {
  ...
  const handleAddClick = () => {
    ...
    setShowSearch(false)
    setNewFloor({ building: "", floor: "" })
    setEditId(null)
  }

  const handleSubmit = () => {
    if (!newFloor.building || !newFloor.floor) return

    if (editId !== null) {
      setData((prev) => prev.map((item) => (item.id === editId ? { ...item, building: newFloor.building } : item)))
    } else {
      const newEntry: Floor = {
        id: Date.now(),
        site: "Vibe",
  ...
  const handleEdit = (item: Floor) => {
    setNewFloor({ building: item.building, floor: "" })
    setEditId(item.id)
    setShowAddForm(true)
    setShowSearch(false)
  }
        ...
        <div className="bg-white mb-6">
          <div className="flex items-center space-x-4">
            <select
              ...
              className="floor-select-building"
            >
            ...
            <input
              ...
              onChange={(e) => setNewFloor({ ...newFloor, floor: e.target.value })}
              className="floor-enter-floor"
            />
