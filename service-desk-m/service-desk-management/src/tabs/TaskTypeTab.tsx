...
export default function TaskTypeTab({ searchQuery: initialSearchQuery = "" }: TaskTypeTabProps) {
  ...
  const handleButtonClick = (buttonType: string) => {
    if (buttonType === "New Task Type") {
      setEditingItem(null) // Clear editing item for new task type
      setFormData({ name: "", description: "", color: "#3b82f6" }) // Clear form data
      setShowModal(true)
    }
  }
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input
            type="color"
            name="viewColor"
            value={viewingItem.color}
            onChange={() => {}} // No-op function since it's read-only
            disabled
            className="w-8 h-8 rounded border border-gray-300 cursor-not-allowed opacity-60"
          />
          <TextInput 
            label=""
            name="viewColorHex" 
            value={viewingItem.color.toUpperCase()} 
            onChange={() => {}} // No-op function since it's read-only
            readOnly
            className="bg-gray-50 font-mono text-sm w-24"
          />
        </div>
      </div>
    </div>
