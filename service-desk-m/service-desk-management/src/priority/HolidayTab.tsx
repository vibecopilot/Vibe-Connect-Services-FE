...
export default function HolidayTab({ searchQuery }: HolidayTabProps) {
  ...
  const viewModalContent = viewingGroup ? (
    <div className="space-y-4" style={{ fontFamily: "PT Sans, sans-serif" }}>
      ...
      />
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700">Applies To</label>
        <div className="w-full p-3 bg-gray-50 rounded-md border border-gray-200 text-gray-600">
          {viewingGroup.appliesTo}
        </div>
      </div>
      <TextInput 
        label="Total Holidays" 
        name="totalHolidays" 
        value={viewingGroup.totalHolidays.toString()} 
        onChange={() => {}} 
        readOnly 
        className="bg-gray-50"
      />
