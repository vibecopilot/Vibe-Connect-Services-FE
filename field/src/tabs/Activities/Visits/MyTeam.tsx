...
import MyTeamForm from "../../../forms/Activities/MyTeamForm"; 
...
interface VisitsMyTeamProps{
  onModalStateChange?: (isOpen: boolean) => void;
}

const MyTeam: React.FC<VisitsMyTeamProps> = ({onModalStateChange}) => { 
  ...
  const [data, setData] = useState<MyTeamItem[]>([]); // Changed from MeItem[]
  const [showFilterRow, setShowFilterRow] = useState(false);
  const [isFormVisible, setIsFormVisible] = useState(false);
  const [formData, setFormData] = useState<Omit<MyTeamItem, "id">>({ // Changed from MeItem
    employeeName: "",
    location: "",
    punchedIn: "",
    activities: 0,
  ...
  const [formErrors, setFormErrors] = useState<Partial<Omit<MyTeamItem, "id">>>({}); // Changed from MeItem
    ...
    let filtered = data.filter((item) =>
      Object.entries(searchFilters).every(([key, val]) => {
        if (key === "activities" || key === "work") {
          return item[key as keyof MyTeamItem]?.toString().includes(val); // Changed from MeItem
        }
        if (key === "missed") {
          if (val === "") return true;
