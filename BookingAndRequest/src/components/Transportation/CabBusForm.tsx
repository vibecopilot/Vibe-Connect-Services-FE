...
const CabBusForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<TransportationRequest>({
    ...
    time: "",
    noOfPassengers: "",
    transportationType: "Cab", // Default for Cab/Bus form
    specialRequirement: "",
    managerApproval: "No",
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (requestToEdit) {
        ...
        navigate("/transportation/cab-bus")
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (isEditMode) {
      console.log("Updating cab/bus request:", formData)
      // In a real app, send PUT/PATCH request to API
    } else {
      console.log("Adding new cab/bus request:", formData)
      // In a real app, send POST request to API
    }
    navigate("/transportation/cab-bus") // Redirect to list page after submission
  }
    ...
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-800">
          {isEditMode ? "Edit Cab/Bus Request" : "Add New Cab/Bus Request"}
        </h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2 flex justify-end gap-3 mt-4">
          <button
            type="button"
            onClick={() => navigate("/transportation/cab-bus")}
