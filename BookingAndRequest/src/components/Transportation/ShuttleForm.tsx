...
const ShuttleForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<TransportationRequest>({
    ...
    time: "",
    noOfPassengers: "",
    transportationType: "Shuttle", // Default for Shuttle form
    specialRequirement: "",
    managerApproval: "No",
  ...
  const mockData: TransportationRequest[] = [] // No mock data for shuttle yet
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (requestToEdit) {
        ...
        navigate("/transportation/shuttle")
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (isEditMode) {
      console.log("Updating shuttle request:", formData)
      // In a real app, send PUT/PATCH request to API
    } else {
      console.log("Adding new shuttle request:", formData)
      // In a real app, send POST request to API
    }
    navigate("/transportation/shuttle") // Redirect to list page after submission
  }

  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        <Link to="/transportation/shuttle" className="text-gray-600 hover:text-gray-800">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h2 className="text-2xl font-bold text-gray-800">
          {isEditMode ? "Edit Shuttle Request" : "Add New Shuttle Request"}
        </h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2 flex justify-end gap-3 mt-4">
          <button
            type="button"
            onClick={() => navigate("/transportation/shuttle")}
