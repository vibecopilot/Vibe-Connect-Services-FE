...
const ReimbursementDisbursementList: React.FC = () => {
  const location = useLocation()
  const { reimbursementDisbursements, loading } = useTravellingAllowanceData()
  ...
  const filteredData = useFilteredData(
    reimbursementDisbursements.map((d) => ({ ...d, status: d.paymentStatus })),
    searchTerm,
    selectedFilter,
  )
  ...
  const handleEdit = (id: string) => {
    console.log(`Editing disbursement with id: ${id}`)
  }

  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to delete this disbursement?")) {
      console.log(`Deleting disbursement with id: ${id}`)
    }
  }
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <Link
                        to={`/travelling-allowance/reimbursement/disbursement/detail/${item.id}`}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.transactionDetails}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
...
export default ReimbursementDisbursementList
